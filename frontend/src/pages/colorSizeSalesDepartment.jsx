...
const tableData = response.data.tableRecords;
  ...
  const fetchDepartmentData = async (row) => {
    setDepartmentTableData([]);
    setDepartmentTableHeaders([]);
    setDepartmentName("");
    ...
    if (firstOption) {
      ...
      const token = localStorage.getItem("authToken");
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}color-size-sales-department-data`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          params: {
              ...
              <div className="flex flex-col w-full space-y-5 mt-10">
                            <div className="overflow-x-auto">
                              <div className="bg-white p-4 border border-gray-300 rounded-md border border-gray-300 min-w-[300px]">
                                ...
                                onRowSelect={(row) => fetchDepartmentData(row)}
                                />

                         <div>
                            {Array.isArray(departmentTableData) && departmentTableData.length > 0 && (
                              <div>
