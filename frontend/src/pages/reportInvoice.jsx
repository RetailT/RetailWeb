...
const Report = () => {
  ...
  const salesReport = async (state, rowClick, companyCodes, invoiceNo, invoiceCompanyCode, invoiceUnitNo, invoiceSalesDate, invoiceRepNo) => {
    setDisable(true);
    const token = localStorage.getItem("authToken");
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}report-data`,
        {
          ...
          params: {
            ...
            currentDate,
            invoiceNo,
            invoiceCompanyCode, // ← added
            invoiceUnitNo, // ← added
            invoiceSalesDate, // ← added
            invoiceRepNo, // ← added
          },
        },
      );
        ...
        if (
          ...
          const formattedInvoiceData = sortedInvoiceData.map((item) =>
            invoiceHeadings.map(({ key }) => {
              ...
              if (key === "TIME") {
                const rawTime = String(item[key]);
                return rawTime.includes("T") ? rawTime.split("T")[1].split(".")[0] : rawTime;
              }
              return item[key];
            }),
          );
  ...
  const filteredReportData = useMemo(() => {
    ...
    return reportData.filter(
      (row) => String(row[unitColIdx]).trim() === unitFilter,
    );
  ...
  const handleRowClick = (headers, rowData) => {
    ...
    const companyIndex = headers.indexOf("Company Code"); // ← added
    const unitIndex = headers.indexOf("Unit Number"); // ← added
    const dateIndex = headers.indexOf("Sales Date"); // ← added
    const repIndex = headers.indexOf("Rep Number"); // ← added

    salesReport(
      false,
      true,
      selectedOptions,
      rowData[invoiceIndex],
      rowData[companyIndex], // ← added
      rowData[unitIndex], // ← added
      rowData[dateIndex], // ← added
      rowData[repIndex], // ← added
    );
    // if (currentSale) {
    //   console.log('1')
    //   if (userData && userData.length > 0) {
