...
setDisable(false);
  ...
  const fetchData = async () => {
    ...
    if (firstOption) {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}vendor-data`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
...
//         const quantityBarChartData = response.data.vendorQuantityBarChart;
  ...
  const fetchVendorData = async (row) => {
    setNewTableData([]);
    setNewTableHeaders([]);
    setDepartmentName("");
      ...
      if (firstOption) {
        ...
        const token = localStorage.getItem("authToken");
        try {
          const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}color-size-sales-vendor-data`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
            params: {
...
setDepartmentName(row.VENDOR_NAME);
              ...
              <div className="overflow-x-auto">
                <div className="bg-white p-4 border border-gray-300 rounded-md border border-gray-300 min-w-[300px]">
                  <NestedDynamicTable
                    ...
                    title="Vendor Sales Data"
                    onRowSelect={(row) => fetchVendorData(row)}
                  />
                   <div>
